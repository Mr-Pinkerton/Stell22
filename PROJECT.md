...
Candidate `audit/08.25` on branch `psr-p2/shadow-activation-control`: **IMPLEMENTED / AWAITING CI / AWAITING HEAD REVIEW / NOT MERGED / NOT RUN IN PRODUCTION / SHADOW INACTIVE**. It adds the manual production control workflow and a separate read-only post-activation verifier. It does not activate SHADOW, does not start dual-write, and does not start PSR-P3. Do not dispatch those workflows from this journal.
...
`audit/08.25-psr-p2-shadow-activation-control.md` records the control-plane candidate: **IMPLEMENTED / AWAITING CI / AWAITING HEAD REVIEW / NOT MERGED / NOT RUN IN PRODUCTION / SHADOW INACTIVE**. Starting `origin/main` `693a9f12fe52b7b6babecdf4f8b6d05e1082f7b6`. Branch `psr-p2/shadow-activation-control`. Production application remains `2d2cbcbdc01ee83cb9103ad8fdaa43ebf8488dcf`. Schema / migration = **NO**. Production accessed = **NO**. Production mutated = **NO**. SHADOW **NOT ACTIVATED**. `production_cost_flow` unchanged. Dual-write **NOT STARTED**. PSR-P3 **NOT STARTED**. `ARCH-P1-001` **OPEN / CONFIRMED**. `INC-001` remains **OPEN — CONTAINMENT DEPLOYED; PHYSICAL FACT REQUIRED FOR DATA CORRECTION**. Gate activation is not dual-write start. Do not merge, deploy, or dispatch the new workflows from this addendum.
