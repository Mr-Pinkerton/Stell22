...
| PSR-P2 SHADOW activation control | Candidate `audit/08.25`. **REWORK IMPLEMENTED / AWAITING HEAD REVIEW / NOT MERGED / NOT RUN IN PRODUCTION / SHADOW INACTIVE**. Previous candidate `329c2be` is **REWORKED / SUPERSEDED**. Current candidate is not recorded as `CI VERIFIED`. `PRODUCTION_ENVIRONMENT_REF_POLICY = UNSAFE`. Not production activation. Dual-write **NOT STARTED**. PSR-P3 **NOT STARTED**. |
...
| `audit/08.25-psr-p2-shadow-activation-control.md` | PSR-P2 SHADOW activation control and start verification | Control-plane rework | Starting main `693a9f12`; previous candidate `329c2be` **REWORKED / SUPERSEDED** | `REWORK IMPLEMENTED / AWAITING HEAD REVIEW / NOT MERGED / NOT RUN IN PRODUCTION / SHADOW INACTIVE` | NO until HEAD accepts | Docker stdin detached. Snapshot and apply tokens required. No automatic rollback. `PRODUCTION_ENVIRONMENT_REF_POLICY = UNSAFE`. No schema. No production mutation. |
...
- Previous candidate `329c2be45dad2a9ab2e69a62e53b9e0000368c6c` is **REWORKED / SUPERSEDED / NOT ACCEPTED**. Its CI `35632872580` remains historical evidence of that tree only.
- This index entry does not call the current candidate `CI VERIFIED`.
